 ...
 */
class lCurrency extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return lCurrency the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('currency_pair, decimals', 'required'),
			array('currency_pair', 'length', 'max'=>128),
			array('decimals', 'length', 'max'=>12),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, currency_pair, decimals', 'safe', 'on'=>'search'),
		);
	}
